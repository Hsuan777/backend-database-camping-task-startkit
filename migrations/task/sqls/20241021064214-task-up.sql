...
join "USER" on "CREDIT_PURCHASE".user_id  = "USER".id
where "USER".email = 'wXlTq@hexschooltest.io'
group by user_id;

-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
select user_id, count(*) as total from "COURSE_BOOKING"
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') and "COURSE_BOOKING".join_at IS NOT NULL
group by user_id;
...
select "COURSE_BOOKING".user_id, sum(purchased_credits) - count(*) as remaining_credit from "COURSE_BOOKING"
inner join "CREDIT_PURCHASE" on "COURSE_BOOKING".user_id = "CREDIT_PURCHASE".user_id
inner join "USER" on "CREDIT_PURCHASE".user_id  = "USER".id
where "USER".email = 'wXlTq@hexschooltest.io' and "COURSE_BOOKING".join_at IS NOT NULL
...
select "USER".name as 教練名稱, experience_years as 經驗年數, "SKILL".name as 專長名稱 from "COACH"
...
where "USER".role = 'COACH' and "SKILL".name  = '重訓'
order by "COACH".experience_years DESC;

-- 6-2 查詢：查詢每種專長的教練數量，並只列出教練數量最多的專長（需使用 group by, inner join 與 order by 與 limit 語法）
-- 顯示須包含以下欄位： 專長名稱, coach_total
select "SKILL".name as 專長名稱, count(skill_id) as coach_total from "COACH_LINK_SKILL"
inner join "SKILL" on skill_id = id
group by "SKILL".name
...
select "CREDIT_PACKAGE".name as 組合包方案名稱, count(credit_package_id) as 銷售數量 from "CREDIT_PURCHASE"
...
-- 顯示須包含以下欄位： 預約會員人數
select distinct count(user_id) as 預約會員人數 from "COURSE_BOOKING"
where extract(month from created_at) = 11 and status != '課程已取消';
